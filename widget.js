/**
 * desv.online CAPTCHA Widget v1.1
...
(function (global) {
  ...
  const API_BASE = "https://illustrious-playfulness-production-dd1f.up.railway.app/v1";
  ...
  const LABELS = {
    en: { enter:"Enter the code above", verify:"Verify", refresh:"Refresh", slide:"Slide to fit the puzzle", verified:"✔ Verified!", wrong:"Wrong, try again", expired:"Expired, refreshed" },
    ar: { enter:"أدخل الكود أعلاه",     verify:"تحقق",  refresh:"تحديث",   slide:"اسحب لملء الشكل",          verified:"✔ تم التحقق!", wrong:"خاطئ، حاول مجدداً",    expired:"انتهت الصلاحية، تم التجديد" },
    fr: { enter:"Entrez le code",        verify:"Vérifier", refresh:"Actualiser", slide:"Glissez pour compléter", verified:"✔ Vérifié!", wrong:"Incorrect",           expired:"Expiré, actualisé" },
    de: { enter:"Code eingeben",         verify:"Verifizieren", refresh:"Aktualisieren", slide:"Schieben zum Ausrichten", verified:"✔ Verifiziert!", wrong:"Falsch, erneut versuchen", expired:"Abgelaufen, erneuert" },
  };
  ...
  const STYLES = `
    ...
    .desv-dark .desv-slider-thumb { background:#7aabff; color:#fff; }
    ...
    .desv-light .desv-slider-thumb { background:#4477dd; color:#fff; }
    ...
    .desv-captcha input {
      ...
      letter-spacing:4px; text-align:center; outline:none; margin-bottom:8px;
    }
    .desv-row { display:flex; gap:6px; }
    .desv-btn { flex:1; padding:9px 0; border:0; border-radius:9px; font-size:12px; font-family:inherit; cursor:pointer; transition:.15s; }
    .desv-refresh { padding:9px 12px; border:0; background:transparent; border-radius:9px; cursor:pointer; font-size:16px; transition:.15s; }
    ...
    /* Puzzle */
    .desv-puzzle-wrap { position:relative; margin-bottom:10px; user-select:none; }
    .desv-puzzle-wrap img.desv-puzzle-bg-img { margin:0; width:100%; border-radius:8px; display:block; }
    .desv-puzzle-piece-img {
      position:absolute; width:50px; height:50px;
      border:2px solid rgba(110,168,255,.55); border-radius:8px;
      overflow:hidden; pointer-events:none;
      box-shadow:0 0 12px rgba(110,168,255,.3), 0 4px 16px rgba(0,0,0,.5);
    }
    .desv-puzzle-piece-img img { width:100%; height:100%; object-fit:cover; display:block; }

    .desv-slider-track { position:relative; height:38px; border-radius:9px; overflow:hidden; margin-bottom:8px; }
    .desv-slider-fill { position:absolute; left:0; top:0; height:100%; width:38px; border-radius:9px 0 0 9px; transition:width .04s; }
    .desv-slider-thumb {
  ...
  async function renderCode(container, opts) {
    ...
    container.innerHTML = `
      <div class="desv-captcha desv-${theme}" style="width:${size.width}">
        ...
        <input id="desv-input" type="text" maxlength="6" placeholder="● ● ● ● ● ●"
    ...
    container.querySelector("#desv-verify-btn").addEventListener("click", async () => {
      ...
      try {
        ...
        container.querySelector("#desv-msg").textContent = "Connection error";
      }
    });
  ...
  async function renderPuzzle(container, opts) {
    ...
    container.innerHTML = `
      <div class="desv-captcha desv-${theme}" style="width:${size.width}">
        ...
        <div class="desv-puzzle-wrap" id="desv-puzzle-wrap">
          <img class="desv-puzzle-bg-img" id="desv-puzzle-bg" src="" alt="puzzle" />
          <div class="desv-puzzle-piece-img" id="desv-puzzle-piece" style="display:none;">
            <img id="desv-piece-inner" src="" alt="">
          </div>
        </div>
    ...
    let currentToken = null;
    let isDragging = false, startX = 0, thumbLeft = 0;
    let pieceY = 0;
    ...
    const msg    = container.querySelector("#desv-msg");
    const pieceEl = container.querySelector("#desv-puzzle-piece");
    ...
    async function loadPuzzle() {
      ...
      try {
        ...
        if (data.success) {
          ...
          const bgImg = container.querySelector("#desv-puzzle-bg");
          bgImg.src = data.background;

          const inner = container.querySelector("#desv-piece-inner");
          inner.src = data.piece;
    ...
    // ✅ prevent default so page doesn't scroll on mobile
    ...
    const end = async () => {
      ...
      // ✅ Correct X calculation matching server answerX
      const maxLeft = track.offsetWidth - 38;
      const bgEl    = container.querySelector("#desv-puzzle-bg");
      const imgW    = bgEl.offsetWidth || 300;
  ...
  async function renderInvisible(container, opts) {
    ...
    const msg   = container.querySelector("#desv-msg");
    let currentToken = null, challenge = null, started = false;
  ...
  const Captcha = {
    /**
     * Render a CAPTCHA widget inside the given element.
     * @param {string|Element} selector - CSS selector or DOM element
     * @param {object} userOpts - Options: { type, theme, size, lang, onSuccess, onError }
     */
    render(selector, userOpts = {}) {
      ...
      if (opts.type === "puzzle")    return renderPuzzle(container, opts);
      if (opts.type === "invisible") return renderInvisible(container, opts);
