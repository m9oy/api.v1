  ...
  async function renderPuzzle(container, opts) {
    ...
    container.innerHTML = `
      <div class="desv-captcha desv-${theme}" style="width:${size.width}">
        ...
        <div class="desv-puzzle-bg">
          <img id="desv-puzzle-bg" src="" alt="puzzle" />
        </div>
    ...
    async function loadPuzzle() {
      ...
      if (data.success) {
        currentToken = data.token;
        puzzleData = data;
        container.querySelector("#desv-puzzle-bg").src = data.background;
      }
    }

    thumb.addEventListener("mousedown", (e) => { isDragging = true; startX = e.clientX - thumbLeft; });
    thumb.addEventListener("touchstart", (e) => { isDragging = true; startX = e.touches[0].clientX - thumbLeft; });
    ...
    const end = async () => {
      ...
      const trackW = track.offsetWidth;
      const imgW = container.querySelector("#desv-puzzle-bg").offsetWidth || 300;
      const submittedX = Math.round((thumbLeft / trackW) * imgW);
